...
top_pos=.95;
bottom_pos=.3;
ncols=floor(ceil/OBJ.settings.gui_rows_per_column);
...
tdt_figure=figure('Visible','off','Name',['TDT interface v.001a'],...
	'Position',[50,50,OBJ.settings.gui_width,min(gui_row_height*ntags,gui_height)],...
	'NumberTitle','off',...
...
for i=1:ntags
	...
	cur_column=floor(rows_per_column/i);
	left_edge=cur_column*column_width+.05;

	name_axis.(tag_names{i})=uicontrol(tdt_figure,'Style','text',...
		'String',tag_names{i},...
		'Units','Normalized',...
		'FontSize',fontsize,...
		'Position',[left_edge cur_pos-box_height text_width box_height/2]);
	...
	tag_axis.(tag_names{i})=uicontrol(tdt_figure,'Style','edit',...
		...
		'Units','Normalized',...
		'FontSize',fontsize,...
		'Position',[pos(1)+pos(3)+.05 cur_pos-box_height edit_width box_height]);
		%'Call',{@mat2tdt_set_tag,dev,tags(i).name});

	cur_pos=cur_pos-box_height-vert_space;
